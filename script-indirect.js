  ...
  //Fetch Instruction
    ...
    if (instruction=="000") {
      ...
      if (operandAddress.substr(0,1)=="@") {  //Indirect Addressing
		  operand = parseInt(document.getElementById("cell_"+operandAddress.substr(1)).value);
	  } else if (operandAddress.substr(0,1)=="#") {  //Direct Addressing
	      direct = true;
		  operand = operandAddress.substr(1);
	  } else {
      ...
      if (opcode=="1") {
		...
		var mdr;
        if (direct)	{
			mdr = parseInt(operand);
			log("   Direct Addressing: Set MDR to the operand of the current instruction: " +operand);
			document.getElementById("mdr").value = mdr;
        } else {
			mdr = parseInt(document.getElementById("cell_"+operand).value);
        ...
        log("   Subtract MDR value from the Accumulator and store the result in the Accumulator: " + accumulator + "-" + mdr +"="+(accumulator-mdr) );
        accumulator -= mdr;
		document.getElementById("acc").value = accumulator;      
      } else if (opcode=="5") {
        //LDA
